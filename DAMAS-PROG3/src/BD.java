...
public class BD {
		...
		try {
			...
			try {
				statement.executeUpdate("create table partida " +
					"(victoria string, fichasFinalA int, fichasFinalB int)");
	...
	public static void partidaInsert( final Statement st, final String victoria, final int fichasFinalA, final int fichasFinalB ) {
		tareasPendientes.add( new Runnable() { @Override public void run() {
			String sentSQL = "";
			try {
				sentSQL = "insert into partida values(" +   // victoria, movimientosA, movimientosB, fichasFinalA, fichasFinalB
						"'" + victoria + "', " +
						fichasFinalA + ", " +
						fichasFinalB + ")";
				System.out.println("inserta partida");
	...
	/*public static String select(Statement st) {
		String sentSQL = "";
		try {
			String ret;
			sentSQL = "select victoria from partida";
